...
(function () {
	...
	function initEventPagination() {
		...
		$eventItems = $eventList.find('.event-card');
		$prevBtn = $('.event-prev-btn');
		$nextBtn = $('.event-next-btn');

		// 설정 읽기
		itemsPerPage = parseInt($eventList.data('items-per-page')) || 4;
	...
	function showPage(pageIndex) {
		...
		// 모든 아이템 숨기기
		$eventItems.addClass('hidden').removeClass('fade-in fade-out');

		// 현재 페이지 아이템만 표시
		for (var i = startIndex; i < endIndex; i++) {
			var $item = $eventItems.eq(i);
	...
	function showEmptyState() {
		if ($eventList.length > 0) {
			$eventList.html('<div class="event-empty"><p>등록된 문화행사가 없습니다.</p></div>');
		}
