...
(function () {
	...
	function initMoviePagination() {
		...
		$movieItems = $movieList.find('.event-card');
		$prevBtn = $('.movie-prev-btn');
		$nextBtn = $('.movie-next-btn');

		// 설정 읽기
		itemsPerPage = parseInt($movieList.data('items-per-page')) || 4;
	...
	function showPage(pageIndex) {
		...
		// 모든 아이템 숨기기
		$movieItems.addClass('hidden').removeClass('fade-in fade-out');

		// 현재 페이지 아이템만 표시
		for (var i = startIndex; i < endIndex; i++) {
			var $item = $movieItems.eq(i);
	...
	function showEmptyState() {
		if ($movieList.length > 0) {
			$movieList.html('<div class="event-empty"><p>등록된 영화상영이 없습니다.</p></div>');
		}
