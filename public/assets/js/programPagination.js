...
(function () {
	...
	function initProgramPagination() {
		...
		$programItems = $programList.find('.event-card');
		$prevBtn = $('.program-prev-btn');
		$nextBtn = $('.program-next-btn');

		// 설정 읽기
		itemsPerPage = parseInt($programList.data('items-per-page')) || 4;
	...
	function showPage(pageIndex) {
		...
		// 모든 아이템 숨기기
		$programItems.addClass('hidden').removeClass('fade-in fade-out');

		// 현재 페이지 아이템만 표시
		for (var i = startIndex; i < endIndex; i++) {
			var $item = $programItems.eq(i);
	...
	function showEmptyState() {
		if ($programList.length > 0) {
			$programList.html('<div class="event-empty"><p>등록된 프로그램이 없습니다.</p></div>');
		}
